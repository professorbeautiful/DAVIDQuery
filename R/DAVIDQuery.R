...
## 
	if(testMe) {
		...
		if(tool == "annotationReport") {
			...
			if(is.numeric(annot)) annot <- DAVIDAnnotChoices[annot]
			if(length(annot) > 1)
	...
	DAVIDQueryResult <- try ({
		...
		firstStageResult <- RCurl::getURL(firstURL, curl=myCurlHandle, verbose=verbose)
		if(writeHTML) writeChar(firstStageResult, "firstStageResult.html")
		DAVIDaction <- bracketedStrings(firstStageResult,
	 		"document.apiForm.action = \"", "\"")
	 	DAVIDvalues <- bracketedStrings(firstStageResult,
	 		"document.apiForm.[a-z]*.value=\"", "\"", warn.if.gt.1=FALSE)
	 	DAVIDfields <- bracketedStrings(firstStageResult, 			"document.apiForm.", ".value=\"", warn.if.gt.1=FALSE)
		secondURL <- paste(DAVIDURLBase, "/", 
			DAVIDaction, "?", 
			paste(DAVIDfields, "=",
			 DAVIDvalues, sep="", collapse="&"), sep="")
		if(verbose) cat("DAVIDQuery:  secondURL = ", secondURL, "\n")
		if(nchar(secondURL) > URLlengthLimit)				stop(paste("nchar(secondURL) too long; ", nchar(secondURL), ">", URLlengthLimit))
		secondStageResult <- RCurl::getURL(secondURL, curl=myCurlHandle, verbose=verbose)
		hasSessionEnded <- length(grep("Your session has ended", secondStageResult) > 0)
		if(hasSessionEnded) warning("Warning: Session ended")
